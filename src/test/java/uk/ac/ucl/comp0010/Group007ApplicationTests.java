...
class Group007ApplicationTests {
  ...
  void testStudentLifecycleEndpoints() throws Exception {
    ...
    Map<String, Object> updatePayload = withPassword(Map.of("firstName", "Updated", "lastName",
        "Name", "userName", "updated" + studentId, "email",
        "updated" + studentId + "@example.com"));

    mockMvc.perform(put("/students/" + studentId).contentType(MediaType.APPLICATION_JSON)
        .content(objectMapper.writeValueAsString(updatePayload))).andExpect(status().isOk())
        .andExpect(jsonPath("$.userName").value("updated" + studentId));
  ...
  void testModuleLifecycleEndpoints() throws Exception {
    ...
    Map<String, Object> updatePayload = withPassword(Map.of("code", "NEW" + moduleId, "name",
        "Updated Module " + moduleId, "mnc", false));

    mockMvc.perform(put("/modules/" + moduleId).contentType(MediaType.APPLICATION_JSON)
        .content(objectMapper.writeValueAsString(updatePayload))).andExpect(status().isOk())
        .andExpect(jsonPath("$.code").value("NEW" + moduleId));
  ...
  void testStudentRegistrationEndpoints() throws Exception {
    ...
    mockMvc.perform(get("/students/" + studentId + "/registrations"))
  ...
  void testRegistrationControllerEndpoints() throws Exception {
    ...
    Map<String, Object> payload = withPassword(Map.of("studentId", studentId, "moduleId", moduleId));
  ...
  void testGradeEndpoints() throws Exception {
    ...
    Map<String, Object> gradePayload = withPassword(
        Map.of("studentId", studentId, "moduleId", moduleId, "score", 75));
    ...
    Grade grade = objectMapper.readValue(createResult.getResponse().getContentAsString(), Grade.class);
    ...
    mockMvc.perform(put("/grades/" + grade.getId()).contentType(MediaType.APPLICATION_JSON)
        .content(objectMapper.writeValueAsString(updatePayload))).andExpect(status().isOk())
  ...
  void testGradeUpsertAndStudentGradeEndpoints() throws Exception {
    ...
    Map<String, Object> studentGradePayload = withPassword(Map.of("moduleId", moduleId, "score", 50));
    ...
    Grade recorded = objectMapper.readValue(gradeResult.getResponse().getContentAsString(), Grade.class);

    Map<String, Object> upsertPayload = withPassword(
        Map.of("studentId", studentId, "moduleId", moduleId, "score", 65));

    mockMvc.perform(post("/grades/upsert").contentType(MediaType.APPLICATION_JSON)
        .content(objectMapper.writeValueAsString(upsertPayload))).andExpect(status().isOk())
  ...
  private Long createStudent() throws Exception {
    ...
    MvcResult result = mockMvc.perform(post("/students").contentType(MediaType.APPLICATION_JSON)
  ...
  private Long createModule() throws Exception {
    int suffix = sequence.incrementAndGet();
    Map<String, Object> req = withPassword(
        Map.of("code", "CODE" + suffix, "name", "Module " + suffix, "mnc", true));

    MvcResult result = mockMvc.perform(post("/modules").contentType(MediaType.APPLICATION_JSON)
        .content(objectMapper.writeValueAsString(req))).andExpect(status().isCreated()).andReturn();
