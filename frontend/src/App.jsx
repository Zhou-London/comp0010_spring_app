...
function App() {
  ...
  const loadAll = async () => {
    ...
    try {
      ...
      showMessage('success', '已刷新数据');
    } catch (error) {
      console.error(error);
      showMessage('error', `获取数据失败：${error.message}`);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleCreateModule = async (event) => {
    ...
    try {
      ...
      showMessage('success', '课程创建成功');
      loadAll();
    } catch (error) {
      console.error(error);
      showMessage('error', `创建课程失败：${error.message}`);
    }
  };
  ...
  const handleRegister = async (event) => {
    ...
    try {
      ...
      showMessage('success', '学生选课成功');
      loadAll();
    } catch (error) {
      console.error(error);
      showMessage('error', `选课失败：${error.message}`);
    }
  };
  ...
  const handleGrade = async (event) => {
    ...
    try {
      ...
      showMessage('success', '成绩已保存');
      loadAll();
    } catch (error) {
      console.error(error);
      showMessage('error', `保存成绩失败：${error.message}`);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <header className="border-b border-slate-200 bg-white">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
          <div>
            ...
            <h1 className="text-2xl font-bold text-slate-900">API 控制面板</h1>
            <p className="text-sm text-slate-600 mt-1">
              基于 openapi.json 生成的简单前端，支持学生、课程、选课与成绩管理。
            </p>
          </div>
          <button
            onClick={loadAll}
      ...
      <main className="mx-auto max-w-6xl space-y-6 px-6 py-6">
        ...
        <SectionCard
          title="连接设置"
          description="可修改接口地址与密码以适配不同的部署环境。"
          actions={
            <span className="text-xs text-slate-500">默认密码 team007（openapi.json 中描述）</span>
          }
        >
        ...
        >
          <form onSubmit={handleCreateStudent} className="grid gap-3 md:grid-cols-4">
            <Input
              ...
              onChange={(event) => setStudentForm({ ...studentForm, firstName: event.target.value })}
              required
            />
            <Input
              label="Last Name"
              value={studentForm.lastName}
              onChange={(event) => setStudentForm({ ...studentForm, lastName: event.target.value })}
              required
            />
            <Input
              label="Username"
              value={studentForm.userName}
              onChange={(event) => setStudentForm({ ...studentForm, userName: event.target.value })}
              required
            />
            <Input
              label="Email"
        ...
        >
          ...
          <Table
            headers={['ID', '代码', '名称', '必修?']}
            rows={modules.map((module) => [module.id, module.code, module.name, module.mnc ? 'Yes' : 'No'])}
          />
        </SectionCard>

        <SectionCard
          title="选课管理"
          description="将学生注册到课程中，也可以用于退课（DELETE /registrations）。"
        ...
        >
          <form onSubmit={handleRegister} className="grid gap-3 md:grid-cols-3">
            ...
            <Input
              ...
              onChange={(event) => setRegistrationForm({ ...registrationForm, moduleId: event.target.value })}
              required
            />
            <div className="flex items-end">
              <button
        ...
        <SectionCard
          title="成绩管理"
          description="通过 /grades/upsert 录入或更新成绩。"
        ...
        >
          <form onSubmit={handleGrade} className="grid gap-3 md:grid-cols-4">
            ...
            <Input
              ...
              onChange={(event) => setGradeForm({ ...gradeForm, moduleId: event.target.value })}
              required
            />
            <Input
              label="分数"
